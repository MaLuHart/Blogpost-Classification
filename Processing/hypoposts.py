...
def prepare_metadatafile(metadatafile, selectedmetadatafile, language):
    """
    Add the required column to the metadatafile.
    Filter out the columns not corresponding to the current language.
    """
    with open(metadatafile, "r") as infile:
        ...
        with open(selectedmetadatafile, "w") as outfile:
            metadata.to_csv(outfile, index=False, sep=";")


def read_metadatafile(selectedmetadatafile):
    """
    Read the metadatafile produced in the previous step.
    """
    with open(selectedmetadatafile, "r") as infile:
        metadata = pd.read_csv(infile, sep=";", index_col=None)
        #print(metadata)       
        return metadata
...
def mark_metadata(metadata, subcollection, selposts):
    """
    Mark all posts corresponding to the current subcollection criteria.
    This is done in the form of a label in the subcollection column.
    """
...
def save_the_metadata(metadata, selectedmetadatafile):
    """
...
def remove_unselected(metadata):
    """
    Filter out all posts not belonging to any subcollection.
    """
    metadata = metadata[metadata["subcollection"] != "none"]
    return metadata
    

    
def build_collection(metadatafile, language, numposts, textlengths, variability):
    selectedmetadatafile = "metadata_selection-" + str(language) + ".csv"
    prepare_metadatafile(metadatafile, selectedmetadatafile, language)
    for textlen in textlengths:
        subcollection = str(language) + "-" + str(textlen)
        print("====", subcollection)
        metadata = read_metadatafile(selectedmetadatafile)
        selposts = identify_posts(metadata, textlen, numposts, variability)
        metadata = mark_metadata(metadata, subcollection, selposts)
        save_the_metadata(metadata, selectedmetadatafile)
    metadata = remove_unselected(metadata)
    save_the_metadata(metadata, selectedmetadatafile)
