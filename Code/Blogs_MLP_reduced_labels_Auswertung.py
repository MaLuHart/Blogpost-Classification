...
# # Auswertung der Klassifikation mit dem MLPClassifier (reduced_labels)
# 
...
# read predicitons
f = open('../MLP/MLP_reduced_labels_predictions.txt', "r", encoding="utf-8")
lines = f.readlines()
f.close()
trues = []
preds = []
...
f = open('../Preprocessing/blogposts_per_reduced_labels_reduced.txt', "r", encoding="utf-8")
lines = f.readlines()
f.close()
...
with open('../MLP/MLP_reduced_labels_prozentuale_auswertung.txt', 'w+', encoding="utf-8") as file:
    file.write("Label (reduced_labels), Trainingsset (#), Testset (#), Vorhersagen (#), Korrekte Vorhersagen (#), Korrekte Vorhersagen (%), Falsche Vorhersagen (#), Falsche Vorhersagen (%), Nicht vorhergesagte Labels (#), Nicht vorhergesagte Labels (%)\n")
