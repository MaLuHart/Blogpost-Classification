...
# # Auswertung der Klassifikation mit dem MLPClassifier (all_labels)
# 
...
# read predicitons
f = open('../MLP/MLP_all_labels_predictions.txt', "r", encoding="utf-8")
lines = f.readlines()
f.close()
trues = []
preds = []
...
f = open('../Preprocessing/blogposts_per_all_labels_reduced.txt', "r", encoding="utf-8")
lines = f.readlines()
f.close()
...
with open('../MLP/MLP_all_labels_prozentuale_auswertung.txt', 'w+', encoding="utf-8") as file:
    file.write("Label (all_labels), Trainingsset (#), Testset (#), Vorhersagen (#), Korrekte Vorhersagen (#), Korrekte Vorhersagen (%), Falsche Vorhersagen (#), Falsche Vorhersagen (%), Nicht vorhergesagte Labels (#), Nicht vorhergesagte Labels (%)\n")
